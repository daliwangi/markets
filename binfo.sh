...
Abbreviations
    ...
    B                Block
    Blk              Block
    BlkTime          Block time (time between blocks)
    BTC              Bitcoin
    Cap              Capital
...
if [[ -n "${SUMMARYOPT}" ]]; then
	...
	"Bal   : \(.[].final_balance) sat  (\(.[].final_balance/100000000) BTC)"'
	exit
fi
# Get RAW ADDR
...
jq -er '.data[].utxo[] | "\t\(.transaction_hash)",
	"\t\t\t\tBlk id:\(.block_id)\tValue:\t\(.value/100000000) BTC"' <<< "${CHAIRADD}" ||
		printf "No unspent tx list.\n"
# Print Address info
jq -r '. | "",
	"Address: \(.data|keys[])",
