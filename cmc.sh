...
# v0.2.3 - 2019/jul/17   by mountaineerbr
...
OPTIONS
		...
		-j 	Fetch JSON of a currency pair ( uses 1 API credit );
			you need FROM_CURRENCY, TO_CURRENCY is optional.
		
		-f 	Fetch full-data JSON; you may set a TO_CURRENCY other
			than USD for the market data ( uses API 12 credits ).
...
# Parse options
while getopts ":blfmhjs:t" opt; do
  case ${opt} in
	...
	h ) # Show Help
		...
		exit 0
		;;
	f ) # Print Full Data JSON
		JSONM=1
		;;
	j ) # Print JSON of a currency pair
		JSONCP=1
		;;
	s ) # Decimal plates
		SCL=${OPTARG}
...
listsf() {
	printf "\n=============CRYPTOCURRENCIES============\n"
	curl -s -H "X-CMC_PRO_API_KEY: 29f3d386-d47d-4b54-9790-278e1faa7cdc" -H "Accept: application/json" -G https://pro-api.coinmarketcap.com/v1/cryptocurrency/map |
...
mcapf() {
	CMCGLOBAL=$(curl -s -H "X-CMC_PRO_API_KEY:  29f3d386-d47d-4b54-9790-278e1faa7cdc" -H "Accept: application/json" -d "convert=USD" -G https://pro-api.coinmarketcap.com/v1/global-metrics/quotes/latest)
...
NOFCUR=(BRL USD CNY JPY CAD NZD AUD EUR GBP LKR RUB VEF XAU XAG)
if [[ -z ${JSONM} ]] &&	printf "%s\n" "${NOFCUR[@]}" | grep -e "${2^^}" &> /dev/null; then
...
## Only grep JSON?
# Print JSON of a currency pair
if [[ -n ${JSONCP} ]]; then
	curl -s -H "X-CMC_PRO_API_KEY: 29f3d386-d47d-4b54-9790-278e1faa7cdc" -H "Accept: application/json" -d "&symbol=${2^^}&convert=${3^^}" -G https://pro-api.coinmarketcap.com/v1/cryptocurrency/quotes/latest | jq .
	exit
elif [[ -n ${JSONM} ]]; then
	curl -s -H "X-CMC_PRO_API_KEY: 29f3d386-d47d-4b54-9790-278e1faa7cdc" -H "Accept: application/json" -d "start=1&limit=5000&convert=${2^^}" -G https://pro-api.coinmarketcap.com/v1/cryptocurrency/listings/latest
	printf "\nMarket quoted in %s.\n" "${2^^}" 1>&2
	exit
fi
...
## Get JSON
CMCJSON=$(curl -s -H "X-CMC_PRO_API_KEY: 29f3d386-d47d-4b54-9790-278e1faa7cdc" -H "Accept: application/json" -d "&symbol=${2^^}&convert=${3^^}" -G https://pro-api.coinmarketcap.com/v1/cryptocurrency/quotes/latest)
## Get pair rate
...
# Spare key:
# KEY: f70ef502-0d91-496b-bd5b-5c0f20334720
...
#cmc() {
#	curl -s -H "X-CMC_PRO_API_KEY: 29f3d386-d47d-4b54-9790-278e1faa7cdc" -H "Accept: application/json" -d "start=1&limit=5000&convert=USD" -G https://pro-api.coinmarketcap.com/v1/cryptocurrency/listings/latest |
#		jq -c '.data[] | [.name,.symbol,.quote.USD.price]' | grep "\"${*^^}\""
#	}


##COINMARKETCAP API
#curl -H "X-CMC_PRO_API_KEY: 29f3d386-d47d-4b54-9790-278e1faa7cdc" -H "Accept: application/json" -d "start=1&limit=5000&convert=USD" -G https://pro-api.coinmarketcap.com/v1/cryptocurrency/listings/latest
#cat ~/TEST |  jq -c '.data[] | [.id ,.name,.quote.USD.price]'
#cat ~/TEST |  jq -c '.data[] | [.name,.symbol,.quote.USD.price]' | grep '"XMR"'
#curl -s -H "X-CMC_PRO_API_KEY: 29f3d386-d47d-4b54-9790-278e1faa7cdc" -H "Accept: application/json" -d "&symbol=XMR&convert=BRL" -G https://pro-api.coinmarketcap.com/v1/cryptocurrency/quotes/latest
