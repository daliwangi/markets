...
mode6() { # Depth of order book (depth=10)
	...
	websocat -nt --ping-interval 20 wss://stream.binance.com:9443/ws/${2,,}${3,,}@depth10 |
		jq -r --arg FCUR "${2^^}" --arg TCUR "${3^^}" '. as $parent | "\nORDER BOOK DEPTH \($FCUR) \($TCUR)",
		"\n\t\($parent.asks[9] | .[0]|tonumber)   	\($parent.asks[9] | .[1]|tonumber)",
		"ASKS\t\($parent.asks[8] | .[0]|tonumber)   	\($parent.asks[8] | .[1]|tonumber)",
		"\t\($parent.asks[7] | .[0]|tonumber)   	\($parent.asks[7] | .[1]|tonumber)",
		"\t\($parent.asks[6] | .[0]|tonumber)   	\($parent.asks[6] | .[1]|tonumber)",
		"\t\($parent.asks[5] | .[0]|tonumber)   	\($parent.asks[5] | .[1]|tonumber)",
		"\t\($parent.asks[4] | .[0]|tonumber)   	\($parent.asks[4] | .[1]|tonumber)",
		"\t\($parent.asks[3] | .[0]|tonumber)   	\($parent.asks[3] | .[1]|tonumber)",
		"\t\($parent.asks[2] | .[0]|tonumber)   	\($parent.asks[2] | .[1]|tonumber)",
		"\t\($parent.asks[1] | .[0]|tonumber)   	\($parent.asks[1] | .[1]|tonumber)",
		"     > \($parent|.asks[0] | .[0]|tonumber)     	\($parent.asks[0] | .[1]|tonumber)",
		"     < \($parent|.bids[0] | .[0]|tonumber)     	\($parent.bids[0] | .[1]|tonumber)",
		"\t\($parent.bids[1] | .[0]|tonumber)   	\($parent.bids[1] | .[1]|tonumber)",
		"\t\($parent.bids[2] | .[0]|tonumber)   	\($parent.bids[2] | .[1]|tonumber)",
		"\t\($parent.bids[3] | .[0]|tonumber)   	\($parent.bids[3] | .[1]|tonumber)",
		"\t\($parent.bids[4] | .[0]|tonumber)   	\($parent.bids[4] | .[1]|tonumber)",
		"\t\($parent.bids[5] | .[0]|tonumber)   	\($parent.bids[5] | .[1]|tonumber)",
		"\t\($parent.bids[6] | .[0]|tonumber)   	\($parent.bids[6] | .[1]|tonumber)",
		"\t\($parent.bids[7] | .[0]|tonumber)   	\($parent.bids[7] | .[1]|tonumber)",
		"BIDS\t\($parent.bids[8] | .[0]|tonumber)   	\($parent.bids[8] | .[1]|tonumber)",
		"\t\($parent.bids[9] | .[0]|tonumber)   	\($parent.bids[9] | .[1]|tonumber)"'
...
mode6extra() { # Depth of order book (depth=20)
	...
	websocat -nt --ping-interval 20 wss://stream.binance.com:9443/ws/${2,,}${3,,}@depth20 |
		jq -r --arg FCUR "${2^^}" --arg TCUR "${3^^}" '. as $parent | "\nORDER BOOK DEPTH \($FCUR) \($TCUR)",
	"\n\t\($parent.asks[19] | .[0]|tonumber)   	\($parent.asks[19] | .[1]|tonumber)",
	"\t\($parent.asks[18] | .[0]|tonumber)   	\($parent.asks[18] | .[1]|tonumber)",
	"\t\($parent.asks[17] | .[0]|tonumber)   	\($parent.asks[17] | .[1]|tonumber)",
	"\t\($parent.asks[16] | .[0]|tonumber)   	\($parent.asks[16] | .[1]|tonumber)",
	"\t\($parent.asks[15] | .[0]|tonumber)   	\($parent.asks[15] | .[1]|tonumber)",
	"\t\($parent.asks[14] | .[0]|tonumber)   	\($parent.asks[14] | .[1]|tonumber)",
	"\t\($parent.asks[13] | .[0]|tonumber)   	\($parent.asks[13] | .[1]|tonumber)",
	"\t\($parent.asks[12] | .[0]|tonumber)   	\($parent.asks[12] | .[1]|tonumber)",
	"\t\($parent.asks[11] | .[0]|tonumber)   	\($parent.asks[11] | .[1]|tonumber)",
	"\t\($parent.asks[10] | .[0]|tonumber)     	\($parent.asks[10] | .[1]|tonumber)",
	"\t\($parent.asks[9] | .[0]|tonumber)   	\($parent.asks[9] | .[1]|tonumber)",
	"ASKS\t\($parent.asks[8] | .[0]|tonumber)   	\($parent.asks[8] | .[1]|tonumber)",
	"\t\($parent.asks[7] | .[0]|tonumber)   	\($parent.asks[7] | .[1]|tonumber)",
	"\t\($parent.asks[6] | .[0]|tonumber)   	\($parent.asks[6] | .[1]|tonumber)",
	...
	"BIDS\t\($parent.bids[8] | .[0]|tonumber)   	\($parent.bids[8] | .[1]|tonumber)",
	"\t\($parent.bids[9] | .[0]|tonumber)   	\($parent.bids[9] | .[1]|tonumber)",
	"\t\($parent.bids[10] | .[0]|tonumber)     	\($parent.bids[10] | .[1]|tonumber)",
	"\t\($parent.bids[11] | .[0]|tonumber)   	\($parent.bids[11] | .[1]|tonumber)",
	"\t\($parent.bids[12] | .[0]|tonumber)   	\($parent.bids[12] | .[1]|tonumber)",
	"\t\($parent.bids[13] | .[0]|tonumber)   	\($parent.bids[13] | .[1]|tonumber)",
	"\t\($parent.bids[14] | .[0]|tonumber)   	\($parent.bids[14] | .[1]|tonumber)",
	"\t\($parent.bids[15] | .[0]|tonumber)   	\($parent.bids[15] | .[1]|tonumber)",
	"\t\($parent.bids[16] | .[0]|tonumber)   	\($parent.bids[16] | .[1]|tonumber)",
	"\t\($parent.bids[17] | .[0]|tonumber)   	\($parent.bids[17] | .[1]|tonumber)",
	"\t\($parent.bids[18] | .[0]|tonumber)   	\($parent.bids[18] | .[1]|tonumber)",
	"\t\($parent.bids[19] | .[0]|tonumber)   	\($parent.bids[19] | .[1]|tonumber)"'
